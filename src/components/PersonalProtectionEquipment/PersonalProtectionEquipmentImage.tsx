...
const PersonalProtectionEquipmentImage = () => {
  ...
  const personalProtectionEquipmentOutputImage = verificationOutputValues?.processedImageUrl
  ...
  return (
    <div className="flex xl:flex-row flex-col items-center mx-auto  gap-10 justify-center my-8" >
      ...
      {isSelectedVerificationFile && !isLoading && <div className="w-[300px] h-auto sm:w-[450px] sm:[400px] flex justify-center items-center">
        {personalProtectionEquipmentUrl !== null && <div className="flex flex-col items-center"><ModalImage small={personalProtectionEquipmentOutputImage} medium={personalProtectionEquipmentOutputImage} alt="Equipment Output Image" className="w-[300px] h-auto" /><p className="text-gray-400">Equipment Output Image</p></div>}
      </div>}
    </div>
  )
}
